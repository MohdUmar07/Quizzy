...
const Exam = ({ userId }) => {
  ...
  useEffect(() => {
    const fetchQuizzes = async () => {
      try {
        ...
        const response = await axios.get(`${process.env.REACT_APP_API_URL}/quiz/allpublishedquiz/exam`, {
          headers: {
  ...
  const handleAttemptQuiz = async (quizId) => {
    try {
      ...
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/exam/${quizId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleSubmitQuiz = async () => {
    try {
      ...
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL}/exam`,
        {
  ...
  const handleToggleFavorite = async (quizId, question) => {
    ...
    try {
      ...
      if (isFavorite) {
        ...
        await axios.delete(`${process.env.REACT_APP_API_URL}/favquestion/${favQuestion.favQuestionId}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
        ...
        const response = await axios.post(
          `${process.env.REACT_APP_API_URL}/favquestion`,
          {
    ...
    <div className="quiz-list">
      {quizzes.map(quiz => (
        <div key={quiz._id} className="quiz-card">
          <h3>{quiz.name}</h3>
          <p>{quiz.category}</p>
          <div className="quiz-actions button-group">
            <button className="attemptbutton" onClick={() => handleAttemptQuiz(quiz._id)}>Attempt Quiz</button>
          </div>
        </div>
      ))}
    </div>
